...
void xuatNhanVien(NhanVien nv)
{
	...
	cout << nv.ma << "\t" << nv.ten << "\t" << nv.tuoi << "\t" << nv.chucVu << "\t" << nv.heSoLuong << "\t" << nv.pb->ten << endl;
}
...
bool ghiDanhSachNhanVien(DanhSachNhanVien ds, char *tenFile)
{
	...
	for (int i = 0; i < dsnv.soLuong; i++)
	{
		fwrite(&ds.ds[i].ma, sizeof(char), 10, f);
		fwrite(&ds.ds[i].ten, sizeof(char), 50, f);
		fwrite(&ds.ds[i].tuoi, sizeof(int), 1, f);
		fwrite(&ds.ds[i].chucVu, sizeof(char), 50, f);
		fwrite(&ds.ds[i].heSoLuong, sizeof(float), 1, f);
		fwrite(&ds.ds[i].pb->ma, sizeof(char), 10, f);
	}
	fclose(f);
	return true;
}
...
bool docDanhSachNhanVien(DanhSachNhanVien &ds, char *tenFile)
{
	...
	for (int i = 0; i < dsnv.soLuong; i++)
	{
		fread(&ds.ds[i].ma, sizeof(char), 10, f);
		fread(&ds.ds[i].ten, sizeof(char), 50, f);
		fread(&ds.ds[i].tuoi, sizeof(int), 1, f);
		fread(&ds.ds[i].chucVu, sizeof(char), 50, f);
		fread(&ds.ds[i].heSoLuong, sizeof(float), 1, f);
		fread(&mapb, sizeof(char), 10, f);
		//Tim mapb trong dspb
		for (int j = 0; j < dspb.soLuong; j++)
			if (strcmp(dspb.ds[j].ma, mapb) == 0)
				ds.ds[i].pb = &dspb.ds[j];
	}
	fclose(f);
	return true;
}
...
int main()
{
	...
	docDanhSachNhanVien(dsnv, "nhanvien2.dat");
	//nhapDanhSachNhanVien(dsnv);
	//ghiDanhSachNhanVien(dsnv, "nhanvien2.dat");
	//strcpy_s(dspb.ds[0].ten, "Hanh chanh");
